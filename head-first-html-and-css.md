...
2.  `<p>、<blockquote>、<ol>、<ul>`和`<li>`都是块元素。他们单独显示，在内容前后分别有一个换行（默认）。
3.  `<q>`和`<em>`是内联元素。这些元素中的内容与其包含元素的其余内容放在一起。
...
8.  要结合两个元素建立一个HTML列表：使用`<ol>`和`<li>`建立有序列表，使用`<ul>`和`<li>`可以建立一个无序列表。
...
11. `<a>`元素的`href`属性中可以使用相对路径或URL来链接到其他Web页面。对于你的网站中的其他页面，最好使用相对路径，对外部链接才使用URL。
12. 可以用id属性在页面中创建一个目标。使用#后面加一个目标id，可以链接到页面中的那个位置。
13. 为了便于访问，可以在`<a>`元素中使用`title`属性提供链接的一个描述。
